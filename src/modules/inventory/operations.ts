...
import { lockInventoryItems, reservedAt } from "./reservations";
...
export async function applyInventoryOperation(
  ...
  return database.$transaction(
    (tx) => applyInventoryOperationInTransaction(tx, input, actorUserId),
    { maxWait: 10_000, timeout: 20_000 },
  );
}

/** Compose ledger changes with a domain transaction, such as receiving a purchase. */
export async function applyInventoryOperationInTransaction(
  ...
  const rows = await lockInventoryItems(tx, [operation.merchandiseItemId]);
  if (!rows.length)
    throw new DomainError("ITEM_NOT_FOUND", "Merchandise item does not exist.");
  // Serialize operation identities after the canonical stock locks.
  ...
  if (locations.length !== locationIds.length)
    throw new DomainError(
      "LOCATION_NOT_FOUND",
      "A storage location does not exist.",
    );
  ...
  if (operation.destinationLocationId) {
    ...
    if (ancestors.some((location) => !location.active)) {
      throw new DomainError(
        "INACTIVE_LOCATION",
        "The destination or an ancestor is inactive.",
      );
    }
  }
  ...
  if (operation.sourceLocationId) {
    const reserved = await reservedAt(
      tx,
      operation.merchandiseItemId,
      operation.sourceLocationId,
    );
    if (quantity + reserved > 2147483647)
      throw new DomainError(
        "INSUFFICIENT_STOCK",
        "Source stock is reserved or insufficient.",
      );
    const removed = await tx.inventoryBalance.updateMany({
      where: {
        ...
        quantity: { gte: quantity + reserved },
      },
    ...
    if (removed.count !== 1)
      throw new DomainError(
        "INSUFFICIENT_STOCK",
        "There is not enough stock available at the source location; reserved units cannot be used.",
      );
  }
  if (operation.destinationLocationId) {
    const identity = {
      merchandiseItemId: operation.merchandiseItemId,
      storageLocationId: operation.destinationLocationId,
    };
  ...
  const movement = await tx.inventoryMovement.create({
    data: { ...operation, requestFingerprint, actorUserId },
  });
...
export async function getOwnedQuantity(
  ...
  const total = await database.inventoryBalance.aggregate({
...
export async function reconcileInventory(
  ...
  return database.$queryRaw<
    { storageLocationId: string; balance: bigint; ledger: bigint }[]
  >`
    WITH deltas AS (
