...
export const inventoryOperationSchema = z
  .object({
    ...
    quantityDelta: databaseInteger.refine(
      (n) => n !== 0,
  ...
  .superRefine((data, ctx) => {
    ...
    if (data.movementType === MovementType.TRANSFER) {
      if (
        ...
        issue(
          "Transfers require distinct source/destination locations and a positive quantity.",
        );
      }
    } else if (
      data.quantityDelta > 0
      ...
      issue(
        "Incoming movements require only a destination; outgoing movements require only a source.",
      );
    }
    if (data.movementType === MovementType.PURCHASE && data.quantityDelta < 0) {
      issue("Purchases must add stock.");
    }
    if (
      [MovementType.SALE, MovementType.DAMAGED, MovementType.LOST].includes(
        data.movementType as "SALE" | "DAMAGED" | "LOST",
      ) &&
      data.quantityDelta > 0
    ) {
    ...
    if (
      (data.acquisitionUnitCostAmount === null) !==
      (data.acquisitionUnitCostCurrency === null)
    ) {
      issue("Acquisition amount and currency must be supplied together.");
    }
    if ((data.referenceType === null) !== (data.referenceId === null))
      issue("Reference type and ID must be supplied together.");
    if (
      data.acquisitionUnitCostAmount !== null &&
      (data.quantityDelta < 0 || data.movementType === MovementType.TRANSFER)
    )
      issue("Record acquisition costs only when receiving stock, never on transfers or removals.");
    if (data.movementType === MovementType.ADJUSTMENT && !data.notes)
      issue("Adjustments require an explanation.");
  });
